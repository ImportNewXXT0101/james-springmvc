...
public class DispatcherServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			*用策略模式实现(把粒度控制得更细),新建 MyHandlerAdapter
			*/
	...
	private void HandlerMapping() {
		...
		for (Map.Entry<String, Object> entry : beans.entrySet()) {
			...
			if (clazz.isAnnotationPresent(MockController.class)) {
				//@com.enjoy.james.annotation.MockRequestMapping(value=/james)
				MockRequestMapping requestMapping = (MockRequestMapping) clazz
						.getAnnotation(MockRequestMapping.class);
				...
				for (Method method : methods) {
					...
					if (method.isAnnotationPresent(MockRequestMapping.class)) {
						//@com.enjoy.james.annotation.MockRequestMapping(value=/query)
						MockRequestMapping methodrm = (MockRequestMapping) method
								.getAnnotation(MockRequestMapping.class);
						String methodPath = methodrm.value();
						// 把方法上与路径建立映射关系( /james/query--->public void com.enjoy.james.controller.MyController.query )
		...
		for (Map.Entry<String, Object> entry : beans.entrySet()) {
			...
			if (clazz.isAnnotationPresent(MockController.class)) {
				...
				for (Field field : fields) {
					if (field.isAnnotationPresent(MockQualifier.class)) {
						MockQualifier qualifier = (MockQualifier) field.getAnnotation(MockQualifier.class);
						//拿到@MockQualifier("MyServiceImpl")里的指定要注入的bean名字"MyServiceImpl"
	...
	private void instance() {
		...
		for (String className : classNames) {
			// com.enjoy.james.service.impl.MyServiceImpl.class
			...
			try {
				...
				if (clazz.isAnnotationPresent(MockController.class)) {
					MockController controller = (MockController) clazz.getAnnotation(MockController.class);
				...
				} else if (clazz.isAnnotationPresent(MockService.class)) {
					//获取当前clazz类的注解(通过这个注解可得到当前service的id)  @com.enjoy.james.annotation.MockService(value=MyServiceImpl)
					MockService service = (MockService) clazz.getAnnotation(MockService.class);
					Object instance = clazz.newInstance();
					//put(MyServiceImpl,instance)
